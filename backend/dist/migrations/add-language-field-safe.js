...
const migrateAddLanguageField = async () => {
    ...
    try {
        ...
        // 检查language字段是否已存在
        const [columns] = await connection.query("SHOW COLUMNS FROM exams LIKE 'language'");
        if (columns.length > 0) {
      ...
      `);
        ...
        // 验证并显示结果
        const [rows] = await connection.query('SELECT id, title, language, created_at FROM exams ORDER BY created_at DESC LIMIT 5');
