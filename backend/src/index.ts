...
import {
  ...
  getQuestionProgressByResultId,
  getResultById,
  getAnswerByResultAndQuestion,
  getResultsByExamId,
  deleteResult,
  deleteResultsByExamId,
...
app.post('/api/users', async (req, res) => {
  try {
    ...
    let user = await getUserByNameAndExam(name, examId);
    if (!user) {
      // 用户不存在，创建新用户（关联到当前考试）
      const userId = await createUser(name, examId);
      user = { id: userId, name, exam_id: examId };
    }
    
    // 检查该用户是否已经参加过这个考试
    const existingResult = await getResultByUserAndExam(user.id!, examId);
...
app.post('/api/answers', async (req, res) => {
  try {
    ...
    // 获取所有有效的正确答案（支持多语言）
    const getValidAnswers = (q: any): string[] => {
      ...
      if (typeof q.correct_answer === 'string' && /^[A-D]$/.test(q.correct_answer)) {
        ...
        if (Array.isArray(q.options)) {
          if (q.options[index]) validAnswers.push(q.options[index]);
        } 
...
app.put('/api/results/:resultId', async (req, res) => {
  try {
    ...
    // 获取用户的所有答案并计算正确数量（服务端计算，更安全）
    const answers = await getAnswersByResultId(resultId);
    const correctCount = answers.filter((a: any) => a.is_correct === 1).length;
    ...
    if (exam.time_limit_minutes && exam.time_limit_minutes > 0) {
      ...
      await updateResultStats(
        resultId,
        correctCount, // 使用服务端计算的分数
        totalQuestions,
        actualElapsedSeconds // 使用服务端计算的时间
      );
    } else {
      // 无时间限制，使用前端传来的时间
      console.log('Updating result stats:', {
...
app.put('/api/results/:resultId/progress', async (req, res) => {
  try {
    ...
    await updateResultProgress(resultId, currentQuestionIndex);
    res.json({ success: true });
  } catch (error) {
...
app.get('/api/results/:resultId/progress', async (req, res) => {
  try {
    const progress = await getQuestionProgressByResultId(Number(req.params.resultId));
    res.json(progress);
  } catch (error) {
...
app.get('/api/results/:resultId', async (req, res) => {
  try {
    const result = await getResultById(Number(req.params.resultId));
    if (!result) {
      return res.status(404).json({ error: 'Result not found' });
    }
    res.json(result);
    ...
    if (exam.time_limit_minutes && exam.time_limit_minutes > 0) {
      ...
      if (isExpired && result.status === 'in_progress') {
        await pool.query(
          ...
          ['expired', elapsedSeconds, resultId]
        );
      }
    }
...
app.delete('/api/exams/:examId/results', async (req, res) => {
  try {
    const examId = Number(req.params.examId);
    await deleteResultsByExamId(examId);
...
app.post('/api/exams/:id/questions/batch', async (req, res) => {
  try {
    ...
    const ids = await createQuestionsBatch(questionsWithExamId);
